"""
Pulls REAL per-zone shooting data for individual Greek Basket League players
from 3stepsbasket.com's player shot chart pages — same source, technique,
and 11-zone template as esake_team_shot_zones_export.py (team-level).
...
Requires data-import/gbl_players_ppg.json (this app's own player_id/name/ppg
per team, exported from the live SQLite DB — see the Node script used to
generate it in this session).
...
Usage:
    python esake_player_shot_zones_export.py --out ./out-esake-player-shot-zones
"""
...
def main():
    ...
    ap.add_argument("--out", type=str, default="./out-esake-player-shot-zones", help="Output folder for CSVs")
    ap.add_argument("--teams", type=str, default=None, help="Comma-separated team slugs to restrict to (default: all 13)")
    ap.add_argument("--players-json", type=str, default="./gbl_players_ppg.json")
    args = ap.parse_args()
